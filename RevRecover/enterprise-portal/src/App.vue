...
const claimsPerPage = 1;
const allClaims = [
  { id: 'CLM-992-81A', date: '2026-02-14', first: 'Eleanor', last: 'Vance', amount: '$12,450.00', status: 'Denied' },
  { id: 'CLM-003-44P', date: '2026-02-20', first: 'Arthur', last: 'Dent', amount: '$430.00', status: 'Denied' },
  { id: 'CLM-814-22X', date: '2026-02-28', first: 'Marcus', last: 'Thorne', amount: '$3,200.00', status: 'Denied' }
];

const paginatedClaims = () => {
  const start = (currentPage.value - 1) * claimsPerPage;
  return allClaims.slice(start, start + claimsPerPage);
};
</script>
        ...
        <div class="glass-card shadow-2xl border-white/60">
          <table class="legacy-table !border-0 !rounded-none !bg-transparent">
            ...
            <tbody class="divide-y divide-slate-100/50">
              <tr v-for="claim in paginatedClaims()" :key="claim.id" class="group hover:bg-white/60 transition-all duration-500 cursor-default">
                <td class="legacy-td pl-12 font-mono font-black text-blue-600 py-8 text-base">#{{ claim.id }}</td>
                <td class="legacy-td text-slate-400 font-bold text-[11px] uppercase tracking-wider py-8">{{ claim.date }}</td>
                <td class="legacy-td font-black text-slate-900 py-8 text-base">{{ claim.last.toUpperCase() }}, {{ claim.first }}</td>
               ...
               <section class="animate-in slide-in-from-bottom-4 duration-700">
                  ...
                  <div class="space-y-8">
                    <div class="p-8 bg-slate-50/50 rounded-[2rem] border border-slate-100 hover:shadow-2xl transition-all duration-700 group cursor-default">
                      ...
                      <p class="text-2xl font-black text-slate-900 group-hover:text-blue-600 transition-colors tracking-tight">Vance, Eleanor</p>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover-lift">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Member ID</p>
                        <p class="text-base font-mono font-black text-slate-900 tracking-tighter">AET88392011</p>
                      </div>
                      <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover-lift">
                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">D.O.B</p>
                        <p class="text-base font-mono font-black text-slate-900 tracking-tighter">1942-05-12</p>
                      </div>
                    </div>
                  </div>
               </section>
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
                   <div class="p-10 rounded-[2.5rem] bg-red-50/20 border border-red-50 shadow-sm relative overflow-hidden group hover:shadow-2xl hover:shadow-red-500/5 transition-all duration-700">
                     ...
                     <div class="flex items-center gap-3 mb-4">
                       <span class="status-badge status-denied px-6 py-2 text-xs">Hard Final Denial</span>
                     </div>
                   ...
                   <div class="p-10 rounded-[2.5rem] bg-slate-900 text-white shadow-3xl relative overflow-hidden group hover:-translate-y-2 transition-all duration-700">
                     ...
                     <p class="text-5xl font-mono font-black tracking-[calc(-0.06em)] text-blue-400 italic mb-3 relative z-10">$12,450<span class="text-2xl font-normal text-slate-600">.00</span></p>
                        ...
                        <div class="p-10 bg-slate-900 rounded-[2.25rem] border border-white/5 relative overflow-hidden group/payload shadow-2xl">
                          ...
                          <div class="space-y-8 relative z-10">
                            <div class="p-8 bg-white/5 rounded-3xl border border-white/5 font-bold text-slate-300 text-lg leading-relaxed shadow-inner font-serif italic">
                               "Claim determination: <span class="text-red-400 italic">EXCLUSION</span>. Adjudicated under RARC-197. Procedure intent requires pre-adjudication authorization protocol. Missing or unverified Prior Auth reference at point of encounter. Submit remediation via Level-1 appeal gateway including certificate mapping."
                            </div>
      ...
      <div v-if="currentScreen === 'success'" class="max-w-3xl mx-auto mt-20 animate-in zoom-in-95 duration-1000">
         <div class="premium-card text-center relative overflow-hidden !p-24 shadow-[0_60px_100px_rgba(0,0,0,0.1)] ring-1 ring-slate-100">
            ...
            <div class="relative z-10">
              ...
              <p class="text-slate-400 mb-16 text-xl font-bold italic max-w-sm mx-auto opacity-70">Documentation payload formally registered in the <span class="text-slate-900 font-black border-b-2 border-blue-600/30">AetnaCare High-Priority Queue</span>.</p>
